...
void readArcs(int arr[MAX][MAX], int m)
{
    ...
    for(int i = 0; i < m; i++)
    {
        ...
        originIndex = find(cityNames.begin(), cityNames.end(), origin) - cityNames.begin();
        destinationIndex = find(cityNames.begin(), cityNames.end(), destination) - cityNames.begin();
        arr[originIndex + 1][destinationIndex + 1] = arr[destinationIndex + 1][originIndex + 1] = cost;
    }
}
            ...
            for(int i = 1; i <= n; i++)
            {
                if(!curr.visited[i] && arr[curr.actualVertex][i] != INT_MAX)
                {
                    ...
                    if(child.level == n - 2)
                    {
                        ...
                        if(arr[i][lastVertex] != INT_MAX && arr[lastVertex][1] != INT_MAX)
                        {
